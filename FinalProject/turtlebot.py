...
class turtlebot():
    ...
    --------------------------------------------------------------------------------------- """
    # # '/map' Callback for updating the map
    ...
    # # Action Status Handler, used for error handling and completion checking on a global scope.
    def _nextAction(self, result, msg):
        if result == 0:
            print msg
            raise NotImplementedError("The Result == 0 occurred and a response is not implemented")
        elif result == 1:
            print "Robot is moving towards it's destination"
        elif result == 2:
            raise NotImplementedError("The result == 2 occurred and a response is not implemented")
        elif result == 3:
            ...
            raise RuntimeError("A case we did not think of has occurred, forget that.")

    """ ---------------------------------------------------------------------------------------
        Frontier Functions : Runs map frontier location functions, set's the frontier
        locations in the turtlebot scope so that the robot can know where to go. This value
    ...
    def startupSpin(self, sleepTime=3, verbose=False):
        ...
        Note: Motion is not actually random, it selects points within a threshold around the robot's current position
        and drives to them.
        :param sleepTime: <int> The duration that the system will sleep in between locations
        :param verbose : <boolean> If the robot will give detailed information while we're driving about.
        :return: None
        """
        ...
        if verbose:
            print "Exiting 'startupSpin' algorithm"
    ...
    def auxillary(self):
        """
        This is an alternate version of main, created in a last ditch effort to meet a deadline.
        It worked, so I guess there's something to be said for code refactoring.
        """
        print "Starting auxiliary"
        while not self.map.doneSetup() and not (rospy.is_shutdown()):
            rospy.sleep(0.1)
        print "System is prepared to start running"
